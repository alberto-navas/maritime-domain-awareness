...
<p align="center">
  <img src="docs/screenshots/map.png" alt="Map generated by the CLI: AIS tracks for several vessels with findings marked by severity (transmission gap, implausible jump, SOG mismatch, sustained encounter)" width="800">
</p>

*Map generated from synthetic demo data (see "Test data" below) — each
line is a vessel, each colored point a finding.*
...
Three independent, separately auditable detectors — `src/detectors/`:
...
50 tests covering all ten pipeline modules (geo, ingest, tracks, config,
zones, all three detectors, pipeline, report, CLI), using versioned
synthetic fixtures in `tests/fixtures/` that follow DMA's real column
schema — none depend on downloading anything external (the real
port-zone extract is versioned too, see "Port zones" below, but it's a
local file, not a test-time download). They run automatically on every
...
would fire on every port in the dataset. For that it uses a real
[OpenStreetMap](https://www.openstreetmap.org/copyright) extract (ODbL
license): 909 port and anchorage polygons (`seamark:type=harbour` /
`seamark:type=anchorage`) across Danish and Baltic waters, versioned in
`data/zones/dk_baltic_ports.geojson`. Only real polygons are included
...
source, the full license text, and how to regenerate or extend the
...
- It does not include an identity-inconsistency detector yet — see
  "Possible extensions".
